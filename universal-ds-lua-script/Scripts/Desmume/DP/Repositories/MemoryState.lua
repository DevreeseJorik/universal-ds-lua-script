...
function MemoryState:setMemoryState(base)
	if memory.readdword(base + self.offsets["memorystate_check"]) == (base + self.values["memorystate_check_value"]) then -- check for ug/bt ptr
...
function MemoryState:update()
    self.base = memory.readdword(memory.readdword(0x2002848)-4)

...
function Game:detect()
    ...
    self.emuDir =  scriptDir .. "/" ..dl.emulator
    self.dir =  self.emuDir .. (gameSelection[self.id] or {dir = ""}).dir
    self.dataDir = self.dir .. "/" .. dataDir
    self.templateDir = self.emuDir .. "/" .. templateDir
...
function Game:runScript()
    ...
    self:runDeSmuMEScript()
end

function Game:runBizHawkScript()
...
function Game:runDeSmuMEScript()
    dofile(self.script) -- extend Game class with detected game's features
    self:init()

    main = function() self:main() end -- gui.register() won't work with self:main directly, blame lua
    gui.register(main)
end
